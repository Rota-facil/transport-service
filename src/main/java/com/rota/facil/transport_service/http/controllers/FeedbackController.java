...
public class FeedbackController {
    ...
    public ResponseEntity<EvaluateUserResponseDTO> evaluateUserByTrip(
            @AuthenticationPrincipal CurrentUser currentUser,
            @Valid @RequestBody EvaluateUserRequestDTO request,
            @PathVariable UUID userId,
            @PathVariable UUID tripId
    ) {
        return ResponseEntity.ok(feedBackService.evaluateByTrip(currentUser, userId, tripId, request));
    }
}
